...
function ProfessionalWorkPage() {
  ...
  const professionalWork = [
    ...
    {
      id: 6,
      title: 'Firmware CI/CD: Build, Versioning & Fleet OTA Operations',
      company: 'iCapo Tech Pvt Ltd (Living Things)',
      period: 'Phase 6',
      description:
      ...
      details: [
        'CI — Automated firmware build and release workflows for the embedded team, covering 17 GSM and 17 WiFi protocol variants, with dependency handling and versioned uploads to Google Cloud Storage.',
        'CI — Reduced firmware release cycle from 2+ hours to around 40 minutes through build automation.',
        'CD — Built a multi-tenant fleet release dashboard with granular filters for Device Type, HW Version, and Library dependencies to control what ships to which devices.',
        'CD — Implemented MAC-ID based targeting for single-device debugging alongside bulk OTA rollout for fleet-wide updates.',
        'CD — Automated release validation before rollout, reducing manual operations overhead by ~70%.',
        'CD — Built real-time OTA deployment tracking and status monitoring so the team can see rollout progress and failures across the entire device network as they happen.',
      ],
    },
    {
      id: 8,
    ...
    },
    {
      ...
      period: 'Phase 25',
      description:
        'Built a live, production-grade Kafka Connect streaming pipeline into ClickHouse for high-volume IoT device event data, with full dead-letter-queue observability and root-caused data-quality handling.',
      technologies: [
        'ClickHouse',
        'Kafka Connect',
        'Kafka',
      ...
      details: [
        ...
        'Diagnosed inconsistent handling of malformed sensor values (e.g. non-numeric strings in numeric fields) across Snowflake and ClickHouse connectors, and designed a schema-level fix using raw-text-plus-materialized-column pairs to safely convert bad values without dropping records.',
        'Validated a real production bulk sync of ~54 million rows into ClickHouse via Airbyte, confirming throughput and stability at scale.',
      ],
    },
    {
      id: 14,
    ...
    },
    {
      ...
      details: [
        'Deployed frontend applications on Bunny.net using storage-account backed pull zones for globally cached delivery.',
        'Wrote Terraform modules for pull zones, storage zones, hostnames, TLS, and cache behaviour so the edge is reproducible from code.',
        'Configured CDN edge rules (caching, redirects, header and origin routing) declaratively instead of clicking through the provider console.',
        'Set up Atlantis so terraform plan runs automatically on every pull request and apply happens only after review and approval.',
        'Eliminated local state drift and shared provider credentials by centralizing runs through Atlantis with remote state.',
        'Gave the team a safe, auditable workflow for infrastructure changes — every edge change is a reviewable diff with a visible plan.',
      ],
    },
    {
      id: 21,
      ...
      details: [
        'Packaged and deployed frontend, backend, databases, object storage, reverse proxy, log streaming, and monitoring as a self-contained stack using Podman.',
        'Designed the stack to run fully offline, with images and dependencies pre-staged for environments that have no outbound connectivity.',
        'Passed customer VAPT alongside Trivy and SonarQube gates — 0 critical CVEs approved for production.',
        'Authored client-handover runbooks and architecture diagrams so customer teams could operate the stack independently.',
      ],
    },
    {
      id: 22,
      title: 'Kubernetes Autoscaling & FinOps Cost Attribution',
      company: 'iCapo Tech Pvt Ltd (Living Things)',
      period: 'Phase 19',
      description:
        'Cut cloud spend and made it accountable — Karpenter for just-in-time batch capacity, OpenCost for per-team cost attribution, and dashboards that put spend in front of the people who cause it.',
      technologies: [
        'Karpenter',
        'OpenCost',
        'Kubernetes',
      ...
      details: [
        'Deployed Karpenter to autoscale nodes for cron and batch workloads, provisioning capacity on demand instead of running idle nodes.',
        'Set up OpenCost for detailed Kubernetes cost reporting with per-namespace and per-team attribution.',
        'Built Looker Studio FinOps dashboards powered by Google Apps Script for automated cost reporting.',
        'Created a dedicated Azure cost dashboard broken down by service name, resource type, and category.',
        'Combined with idle VM suspension and right-sizing, reduced overall cloud costs by ~20%.',
      ],
    },
    {
      id: 23,
      ...
      details: [
        'Deployed CloudNativePG for a partner team with connection pooler setup and initial-restore bootstrap from existing backups.',
        'Configured Barman Cloud for continuous backups and point-in-time recovery of CloudNativePG clusters.',
        'Enabled pgAudit for query-level auditing to support security review and compliance needs.',
        'Built Grafana dashboards and alerts for cluster health, replication state, and PVC capacity trends.',
        'Ran pgBackRest (full/incremental/differential + PITR) on the primary estate achieving RPO under 1 minute, with Databasus for centralized backup management and unified restore workflows.',
      ],
    },
    {
      id: 24,
      ...
      details: [
        'Owned planning, design, proposal, development, deployment, and validation for 3 high-priority projects without hand-off.',
