  ...
  const professionalWork = [
    ...
    },
    {
      ...
      details: [
        ...
        { text: 'Consolidated multiple paid per-service LoadBalancers into a single self-hosted frp tunnel, cutting recurring LoadBalancer costs (see frp Tunnel Setup for the full breakdown).', icon: BsCashCoin },
    ...
    },
    {
      ...
      details: [
        { text: 'Deployed frps (frp server) in-cluster with a single LoadBalancer service as the one public entry point, replacing what would otherwise be a separate LoadBalancer per exposed service.', icon: BsSignpost2Fill },
        { text: 'Deployed frpc (frp client) in-cluster, tunneling traffic internally to each target service via its cluster DNS name — no external VM needed for either component.', icon: BsRouterFill },
        { text: 'Secured the frpc-to-frps control connection with a shared token stored as a Kubernetes Secret, keeping it out of plaintext configs.', icon: BsShieldLockFill },
        { text: 'Exposed the app, ClickHouse, and Postgres (via PgBouncer) through the single frp public IP on dedicated ports, verified working end to end.', icon: SiClickhouse },
        { text: 'Kept the old per-service LoadBalancers running in parallel during validation, with a clear cutover plan to remove them (and realize the cost savings) once the team confirms the frp endpoints.', icon: BsCashCoin },
      ],
    },
    {
      id: 17,
