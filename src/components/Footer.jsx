...
const navigation = {
  social: [
    ...
    {
      name: "Instagram",
      href: "https://www.instagram.com/Kishlay_chandan0129/",
      icon: (props) => (
        <svg fill="currentColor" viewBox="0 0 24 24" {...props}>
          <path
            fillRule="evenodd"
    ...
    <footer className={`${isDark ? 'bg-slate-950 border-t border-slate-900' : 'bg-slate-900'} transition-colors duration-300`}>
      <div className="flex justify-center flex-col items-center mx-auto max-w-7xl overflow-hidden px-6 py-20 sm:py-24 lg:px-8">
            ...
            return (
              <div key={index}>
                <button
                  ...
                  className="text-sm leading-6 text-gray-400 hover:text-white cursor-pointer transition-colors duration-200 font-medium"
                >
                  {item.name}
                </button>
              </div>
        ...
        <p className="mt-10 text-center text-xs leading-5 text-gray-500">
          &copy; {new Date().getFullYear()} Kishlay Chandan. All rights reserved.
        </p>
      </div>
    </footer>
  );
